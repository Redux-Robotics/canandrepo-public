...
//! ```
use crate::{
    frame::AsPtr, CanAddr, CanAnyFrame, CanFdFrame, CanFrame, Error, IoResult, Result, Socket, SocketOptions,
    socket::TimestampingMode,
};
...
impl<T: Socket + From<OwnedFd>> AsyncCanSocket<T> {
    ...
    /// Gets a handle to the inner (blocking) object.
    pub fn inner(&self) -> &T {
        self.0.get_ref()
    }
}
